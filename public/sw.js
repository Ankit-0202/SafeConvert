...
const CACHE_VERSION = "safeconvert-v10";
      ...
      .then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cacheName) => {
            if (cacheName.startsWith("safeconvert-") && cacheName !== CACHE_NAME) {
...
self.addEventListener("fetch", (event) => {
  ...
  if (
    isLocal &&
    (url.searchParams.has("t") || url.searchParams.has("import") || url.searchParams.has("direct"))
  ) {
