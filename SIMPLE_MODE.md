...
```bash
# Docker
docker run -p 3000:8080 ghcr.io/alam00000/safeconvert-simple:latest

# Podman
podman run -p 3000:8080 ghcr.io/alam00000/safeconvert-simple:latest
```
...
# Podman
podman run -p 3000:8080 docker.io/safeconvertteam/safeconvert-simple:latest
```
...
services:
  safeconvert:
    ...
    # Or using Docker Hub
    # image: safeconvertteam/safeconvert-simple:latest
...
```bash
docker build --build-arg SIMPLE_MODE=true -t safeconvert-simple .
docker run -p 3000:8080 safeconvert-simple
```
...
docker pull ghcr.io/alam00000/safeconvert-simple:latest
docker run -p 3000:8080 ghcr.io/alam00000/safeconvert-simple:latest

# Podman
podman pull ghcr.io/alam00000/safeconvert-simple:latest
podman run -p 3000:8080 ghcr.io/alam00000/safeconvert-simple:latest
```
...
docker run -p 3000:8080 ghcr.io/alam00000/safeconvert:latest

# Test Simple Mode (Docker)
docker run -p 3001:8080 ghcr.io/alam00000/safeconvert-simple:latest
...
- `ghcr.io/alam00000/safeconvert:v1.0.0` (versioned)

**Docker Hub:**

- `safeconvertteam/safeconvert:latest`
- `safeconvertteam/safeconvert:v1.0.0` (versioned)

### Simple Mode (Clean Interface)

**GitHub Container Registry (Recommended):**

- `ghcr.io/alam00000/safeconvert-simple:latest`
- `ghcr.io/alam00000/safeconvert-simple:v1.0.0` (versioned)

**Docker Hub:**

- `safeconvertteam/safeconvert-simple:latest`
- `safeconvertteam/safeconvert-simple:v1.0.0` (versioned)
...
services:
  safeconvert:
    image: ghcr.io/alam00000/safeconvert-simple:latest # Recommended
    # image: safeconvertteam/safeconvert-simple:latest     # Alternative: Docker Hub
    container_name: safeconvert
    restart: unless-stopped
    ports:
      - "80:8080"
...
Create `~/.config/containers/systemd/safeconvert-simple.container`:

```ini
[Unit]
Description=SafeConvert Simple Mode
...
systemctl --user enable --now safeconvert-simple
```

## ⚠️ Important Notes

- **Pre-built images**: Use `ghcr.io/alam00000/safeconvert-simple:latest` for Simple Mode (recommended)
