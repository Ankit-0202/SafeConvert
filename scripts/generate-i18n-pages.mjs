...
const SITE_URL = process.env.SITE_URL || "https://safeconvert.vercel.app";
...
function loadAllTranslations() {
  ...
  for (const lang of languages) {
    ...
    translations[lang] = {
      common: fs.existsSync(commonPath) ? JSON.parse(fs.readFileSync(commonPath, "utf-8")) : {},
      tools: fs.existsSync(toolsPath) ? JSON.parse(fs.readFileSync(toolsPath, "utf-8")) : {},
    };
  }
  return translations;
}
...
function processFileForLanguage(originalContent, file, lang, translations, langDir) {
  ...
  if (tools[translationKey]) {
    title =
      ...
      (tools[translationKey].name ? `${tools[translationKey].name} - SafeConvert` : null);
  ...
  links.forEach((link) => {
    ...
    const langPrefixRegex = new RegExp(`^(${BASE_PATH})?/(${languages.join("|")})(/|$)`);
    if (langPrefixRegex.test(href)) return;

    let newHref;
    if (href.startsWith("/")) {
      const pathWithoutBase = href.startsWith(BASE_PATH) ? href.slice(BASE_PATH.length) : href;
...
async function generateI18nPages() {
  ...
  for (const file of htmlFiles) {
    ...
    for (const lang of languages) {
      ...
      processFileForLanguage(originalContent, file, lang, translations, langDir);
