...
const init = async () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      document.title = "SafeConvert - PDF Tools";
  ...
  if (dom.toolGrid) {
    ...
    categories.forEach((category) => {
      ...
      title.className = "category-title text-xl md:text-2xl font-semibold";
    ...
    searchBar.addEventListener("input", () => {
      ...
      categoryGroups.forEach((group) => {
        ...
        toolCards.forEach((card) => {
          ...
          const toolHref =
            (card as HTMLAnchorElement).href || (card as HTMLElement).dataset.toolId || "";
