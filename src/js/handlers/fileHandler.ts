...
async function handleSinglePdfUpload(toolId, file) {
  ...
  try {
    ...
    if (
      ["split", "delete-pages", "add-blank-page", "extract-pages", "add-header-footer"].includes(
        toolId,
      )
    ) {
    ...
    if (toolId === "edit-metadata") {
      ...
      (document.getElementById("meta-mod-date") as HTMLInputElement).value = formatDateForInput(
        state.pdfDoc.getModificationDate(),
      );
...
async function handleMultiFileUpload(toolId) {
  if (toolId === "merge" || toolId === "alternate-merge" || toolId === "reverse-pages") {
    ...
    const foundEncryptedPDFs = pdfFilesLoaded.filter((pdf) => pdf.pdfDoc.isEncrypted);
...
export function setupFileInputHandler(toolId) {
  ...
  const processFiles = async (newFiles) => {
    ...
    if (isMultiFileTool) {
      ...
      const optionsDivId =
        toolId === "change-permissions" ? "permissions-options" : `${toolId}-options`;
