...
import type { OcrState, OcrLine, OcrPage } from "@/types";
import {
  parseHocrDocument,
...
async function runOCR() {
  const selectedLangs = Array.from(document.querySelectorAll(".lang-checkbox:checked")).map(
    function (cb) {
  ...
  if (langSearch && langList) {
    langSearch.addEventListener("input", function () {
      ...
      langList.querySelectorAll("label").forEach(function (label) {
        (label as HTMLElement).style.display = label.textContent?.toLowerCase().includes(searchTerm)
          ? ""
          : "none";
      });
    });
