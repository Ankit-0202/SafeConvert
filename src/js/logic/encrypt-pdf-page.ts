...
import type { EncryptPdfState } from "@/types";
...
async function encryptPdf() {
  ...
  const userPassword =
    (document.getElementById("user-password-input") as HTMLInputElement)?.value || "";
  const ownerPasswordInput =
    (document.getElementById("owner-password-input") as HTMLInputElement)?.value || "";
  ...
  try {
    ...
    try {
      ...
      throw new Error("Encryption failed: " + (qpdfError.message || "Unknown error"));
    }
