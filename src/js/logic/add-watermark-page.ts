...
import type { AddWatermarkState } from "@/types";
...
function setupWatermarkUI() {
  ...
  opacitySliderText?.addEventListener("input", () => {
    if (opacityValueText) opacityValueText.textContent = opacitySliderText.value;
  });
  ...
  opacitySliderImage?.addEventListener("input", () => {
    if (opacityValueImage) opacityValueImage.textContent = opacitySliderImage.value;
  });
...
async function addWatermark() {
  ...
  const watermarkType =
    (document.querySelector('input[name="watermark-type"]:checked') as HTMLInputElement)?.value ||
    "text";
  ...
  try {
    ...
    if (watermarkType === "text") {
      ...
      const imageFile = (document.getElementById("image-watermark-input") as HTMLInputElement)
        .files?.[0];
    ...
    for (const page of pages) {
      ...
      if (watermarkType === "text") {
        ...
        const angle =
          parseInt((document.getElementById("angle-text") as HTMLInputElement).value) || 0;
        const opacity =
