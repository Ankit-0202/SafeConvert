...
function getGridDimensions(): { rows: number; cols: number } {
  const gridMode =
    (document.querySelector('input[name="grid-mode"]:checked') as HTMLInputElement)?.value || "1x2";
  switch (gridMode) {
    case "1x2":
...
function getSheetDimensions(isBookletMode: boolean): { width: number; height: number } {
  const paperSizeKey = (document.getElementById("paper-size") as HTMLSelectElement)
...
async function generatePreview() {
  ...
  const rotationMode =
    (document.querySelector('input[name="rotation"]:checked') as HTMLInputElement)?.value || "none";
