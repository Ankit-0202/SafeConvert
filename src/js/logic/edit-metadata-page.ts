import type { EditMetadataState } from "@/types";
...
function populateMetadataFields() {
  ...
  if (creationDateInput)
    creationDateInput.value = formatDateForInput(pageState.pdfDoc.getCreationDate());
  ...
  try {
    ...
    const allKeys = infoDict.keys().map(function (key: { asString: () => string }) {
