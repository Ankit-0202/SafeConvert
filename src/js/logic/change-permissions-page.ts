...
import type { ChangePermissionsState } from "@/types";
...
async function changePermissions() {
  ...
  const newOwnerPassword =
    (document.getElementById("new-owner-password") as HTMLInputElement)?.value || "";
  ...
  try {
    ...
    if (shouldEncrypt) {
      ...
      const allowAnnotating = (document.getElementById("allow-annotating") as HTMLInputElement)
        ?.checked;
      const allowFillingForms = (document.getElementById("allow-filling-forms") as HTMLInputElement)
        ?.checked;
    ...
    try {
      ...
      if (errorMsg.includes("encrypted") || errorMsg.includes("password required")) {
