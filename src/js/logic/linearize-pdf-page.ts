...
import type { LinearizePdfState } from "@/types";
...
async function linearizePdf() {
  const pdfFiles = pageState.files.filter((file: File) => file.type === "application/pdf");
  ...
  try {
    ...
    showAlert("Linearization Failed", `An error occurred: ${error.message || "Unknown error"}.`);
  } finally {
