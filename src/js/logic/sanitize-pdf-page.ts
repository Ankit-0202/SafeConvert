...
import type { SanitizePdfState } from "@/types";
...
async function sanitizePdf() {
  ...
  try {
    ...
    const shouldFlattenForms = (document.getElementById("flatten-forms") as HTMLInputElement)
      .checked;
    const shouldRemoveMetadata = (document.getElementById("remove-metadata") as HTMLInputElement)
      .checked;
    ...
    const shouldRemoveLayers = (document.getElementById("remove-layers") as HTMLInputElement)
      .checked;
    ...
    const shouldRemoveMarkInfo = (document.getElementById("remove-markinfo") as HTMLInputElement)
      .checked;
      ...
      try {
        ...
        for (const page of pages) {
          try {
            ...
            console.warn(`Could not process page for attachments: ${pageError.message}`);
          }
        }
                  ...
                  try {
                    ...
                    for (const fontKey of fontKeys) {
                      try {
                        ...
                        const specificFont = pdfDoc.context.lookup(specificFontRef) as any;

                        if (specificFont.has(PDFName.of("FontDescriptor"))) {
                          const descriptorRef = specificFont.get(PDFName.of("FontDescriptor"));
                          const descriptor = pdfDoc.context.lookup(descriptorRef) as any;
    ...
    if (!changesMade) {
      showAlert("No Changes", "No items were selected for removal or none were found in the PDF.");
