...
import { getWasmBaseUrl, fetchWasmFile, isWasmAvailable } from "../config/wasm-cdn-config.js";
...
export async function convertToPdfA(
  ...
  if (!isWasmAvailable("ghostscript")) {
    throw new Error("Ghostscript is not configured. Please configure it in WASM Settings.");
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch ICC profile from ${iccUrl}: HTTP ${response.status}`);
    }
