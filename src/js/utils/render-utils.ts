...
async function renderPageBatch(
  ...
  for (const task of tasks) {
    try {
      const canvas = await renderPageToCanvas(task.pdfjsDoc, task.pageNumber, task.scale || 0.5);
      ...
      if (placeholder && placeholder.parentNode) {
        ...
        const allChildren = Array.from(task.container.children) as HTMLElement[];
 ...
 */
function setupLazyRendering(container: HTMLElement, config: RenderConfig): IntersectionObserver {
  const options = {
...
export async function renderPagesProgressively(
  ...
  if (useLazyLoading && totalPages > initialRenderCount) {
    ...
    const eagerEndIndex = Math.min(eagerStartIndex + eagerLoadBatches * batchSize, totalPages);
