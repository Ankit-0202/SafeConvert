...
>
> The official SafeConvert nginx images include these headers. In Kubernetes, **Ingress/Gateway controllers are also reverse proxies**, so ensure these headers are preserved (or add them at the edge).

## Prereqs

- Kubernetes cluster
- Helm v3
- A SafeConvert nginx image (e.g. `ghcr.io/alam00000/safeconvert:<tag>`) that serves on **port 8080**
...
```bash
kubectl create namespace safeconvert

helm upgrade --install safeconvert /path/to/safeconvert/chart \
  --namespace safeconvert \
  --set image.repository=ghcr.io/alam00000/safeconvert \
...
helm upgrade --install safeconvert oci://ghcr.io/$GHCR_USERNAME/charts/safeconvert \
  --namespace safeconvert \
  --create-namespace \
  --version 0.1.0 \
  --set image.repository=ghcr.io/alam00000/safeconvert \
...
```bash
kubectl -n safeconvert port-forward deploy/safeconvert 8080:8080
```
...
SafeConvert’s nginx config sets the required response headers. Most Ingress/Gateway controllers **pass upstream response headers through unchanged**.
