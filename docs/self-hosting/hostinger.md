...
[Hostinger](https://hostinger.com) is a popular shared hosting provider. This guide covers deploying SafeConvert to Hostinger's shared hosting.
...
RewriteCond %{HTTP_HOST} ^safeconvert\.com [NC]
RewriteRule ^(.*)$ https://www.safeconvert.com/$1 [L,R=301]
...
Make sure the `.html` extension rule and language routes are correctly configured. SafeConvert uses static HTML files, not SPA routing:
