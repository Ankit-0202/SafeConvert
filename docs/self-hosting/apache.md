...
Host SafeConvert using Apache HTTP Server.
...
```bash
sudo mkdir -p /var/www/safeconvert
sudo cp -r dist/* /var/www/safeconvert/
sudo chown -R www-data:www-data /var/www/safeconvert
```

## Step 3: Apache Configuration

Create `/etc/apache2/sites-available/safeconvert.conf`:
...
<VirtualHost *:443>
    ServerName your-domain.com
    DocumentRoot /var/www/safeconvert

    SSLEngine on
    SSLCertificateFile /etc/letsencrypt/live/your-domain.com/fullchain.pem
    SSLCertificateKeyFile /etc/letsencrypt/live/your-domain.com/privkey.pem

    <Directory /var/www/safeconvert>
...
Create `/var/www/safeconvert/.htaccess`:
...
```bash
sudo a2ensite safeconvert.conf
...
```bash
sudo chown -R www-data:www-data /var/www/safeconvert
sudo chmod -R 755 /var/www/safeconvert
```
