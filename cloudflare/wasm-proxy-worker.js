...
const ALLOWED_ORIGINS = ["https://safeconvert.vercel.app", "https://safeconvert.vercel.app"];
  ...
  try {
    ...
    if (!response) {
      response = await fetch(targetUrl, {
        headers: {
          "User-Agent": "SafeConvert-WASM-Proxy/1.0",
          Accept: "*/*",
        },
      });
  ...
  async fetch(request, env, ctx) {
    ...
    if (!isAllowedOrigin(origin)) {
      return new Response(
        JSON.stringify({
          error: "Forbidden",
          message: "Origin not allowed. Add your domain to ALLOWED_ORIGINS if self-hosting.",
        }),
        {
          status: 403,
          headers: {
    ...
    if (pathname === "/" || pathname === "/health") {
      return new Response(
        JSON.stringify({
          service: "SafeConvert WASM Proxy",
          version: "1.0.0",
          endpoints: {
            pymupdf: "/pymupdf/*",
            gs: "/gs/*",
